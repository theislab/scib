...
suppressWarnings({
 ...
 ####################################
  ...
  # circle legend
  if (any(column_pos$geom == "circle")) {
    cir_minimum_x <- column_pos %>% filter(geom == "text") %>% pull(xmin) %>% min
    ...
    cir_legend_maxy <- legend_pos - 2.5
    ...
    cir_title_data <-
      data_frame(xmin = cir_minimum_x, xmax = cir_maximum_x, ymin = legend_pos - 1.5, ymax = legend_pos - .5, label_value = "Score legend", hjust = 0, vjust = 1, fontface = "bold")
    ...
    cir_value_data <- data.frame(
        ...
        vjust = 0,
        size = 2,
        label_value = ifelse(cir_legend_dat$value %in% c(0, 1), sprintf("%.0f", cir_legend_dat$value), sprintf("%.1f", cir_legend_dat$value))
      )
