...
# ind_full <- grep("full", rownames(metrics_tab))
# metr_full <- metrics_tab[ind_full,]
# 
# ind_embed <- grep("embed", rownames(metrics_tab))
# metr_embed <- metrics_tab[ind_embed,]
# 
# ind_knn <- grep("knn", rownames(metrics_tab))
# metr_knn <- metrics_tab[ind_knn,]

# order methods by the overall score
if(length(unique(score_all))!= 1){
  # metr_full <- metr_full[order(metr_full$`Overall Score`, decreasing = T), ]
  # metr_embed <- metr_embed[order(metr_embed$`Overall Score`, decreasing = T), ]
  # metr_knn <- metr_knn[order(metr_knn$`Overall Score`, decreasing = T), ]
...
plot_table_metrics <- function(metrics_tab){ 
  ...
  palettes <- list("Score overall" = colorRampPalette(rev(brewer.pal(nrow(metrics_tab), "YlGnBu")))(nrow(metrics_tab)),
                 "Removal of batch effects" = colorRampPalette(rev(brewer.pal(nrow(metrics_tab), "BuPu")))(nrow(metrics_tab)),
                 "Cell type label variance" = colorRampPalette(rev(brewer.pal(nrow(metrics_tab), "RdPu")))(nrow(metrics_tab))
)
...
if (nrow(rect_data) > 0) {
  ...
  g <- g + geom_rect(aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = colors, colour = border_colour, alpha = alpha), rect_data, size = .25, show.legend = T)
}
...
row_groups <- data.frame(name = method_group, group = method_group)
