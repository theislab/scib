...
plotSingleAtlasATAC <- function(csv_file_path){
  ...
  metrics <- plyr::mapvalues(metrics, from = c("ASW label", "ASW label/batch", "cell cycle conservation", "hvg overlap"), 
                             to = c("Cell type ASW", "Batch ASW", "CC conservation", "HVG conservation"))
  ...
  for (dt.sc in data.scenarios){
    ...
    column_info <- data.frame(id = colnames(metrics_tab),
                              ...
                              width = c(3.5,2.5,2,2, rep(1,n_metrics_batch), 2, rep(1,n_metrics_bio)),
                              overlay = F)
